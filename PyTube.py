...
def get_playlists(playlists):
    ...
    for num, link in enumerate(playlists):
        ...
        # Se for válida, guarda o nome, no indíce o da lista, e outra lista dos vídeos no indíce 1:
            ...
            while True:
                try:
                    video = driver.find_element('xpath', f'/html/body/ytd-app/div[1]/ytd-page-manager/ytd-browse/ytd-two-column-browse-results-renderer/div[1]/ytd-section-list-renderer/div[2]/ytd-item-section-renderer/div[3]/ytd-playlist-video-list-renderer/div[3]/ytd-playlist-video-renderer[{count}]/div[2]/div[1]/ytd-thumbnail/a').get_attribute('href') #noqa
...
for play in output_playlists: # noqa
        ...
        # Se não tiver nenhum erro, tenta acessa-lo com a qualidade escolhida:
            ...
            for i in range(6):
                ...
                # Se não der, troca a resolução:
                    ...
                    resol_video = resolutions[resolutions.index(resol) + resol_round]
                    video_download = YouTube(video).streams.get_by_resolution(resol_video)
